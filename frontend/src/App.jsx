...
function App() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:5000/api/addTodo", addTodo)
      .then(() => {
        setFlag(flag + 1);
        setShowHide(false);
      })
  ...
  const handleUpdate = (e) => {
    e.preventDefault();
    axios
      .patch("http://localhost:5000/api/update/" + editId, editTodo)
      .then(() => {
        setFlag(flag + 1);
        setShowEdit(false);
      })
  ...
  return (
    <>
      ...
      <h1 className="text-center text-xl underline">
        <button
          onClick={() => setShowHide(!show)}
          className="border-black border p-2 rounded bg-green-500 mt-5"
        >
          Add
        </button>
      </h1>
      ...
      {show && (
        <div className="h-screen w-full absolute flex items-center justify-center">
          <div className="border bg-green-200 border-black p-4">
            ...
            <textarea
              onChange={(e) =>
                setAddTodo({ ...addTodo, content: e.target.value })
              }
              name="content"
              rows={5}
              cols={50}
      ...
      {showEdit && (
        <div className="h-screen w-full absolute flex items-center justify-center">
          <div className="border bg-green-200 border-black p-4">
            ...
            ></textarea>
            <br />
            <button onClick={handleUpdate} className="bg-green-500 p-2 rounded">
              Edit Todo
            </button>
            <button
          ...
          <div key={k} className="border border-black p-2">
            ...
            <div className="flex justify-end">
              <button
                ...
                className="border border-black p-2 m-1 text-white rounded bg-red-500"
              >
                Delete
              </button>
              <button
                onClick={() => handleEdit(t.id)}
                className="border border-black p-2 m-1 text-white rounded bg-blue-500"
              >
                Edit
              </button>
            </div>
